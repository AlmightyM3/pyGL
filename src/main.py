...
if __name__ == "__main__":
    ...
    blankNormal = Texture(f"{dirPath}/assets/normal.jpg")
    ...
    rockDiffuse = Texture(f"{dirPath}/assets/rock-wall/rock-wall_albedo.png", type=GL.GL_RGB)
    rockSpecular = Texture(f"{dirPath}/assets/rock-wall/rock-wall_metallic.png", type=GL.GL_RGB)
    rockNormal = Texture(f"{dirPath}/assets/rock-wall/rock-wall_normal-ogl.png", type=GL.GL_RGB)

    rootNode = Node("Root Node")

    cubeNode1 = RenderNode(suzanneMesh,litShader,blankTex,blankTex,blankNormal, "Suzanne")
    cubeNode2 = RenderNode(cubeMesh,litShader,containerTex,containerSpecularTex,blankNormal, "Box 0")
    cubeNode3 = RenderNode(cubeMesh,litShader,containerTex,containerSpecularTex,blankNormal, "Box 1")
    ...
    teapot = RenderNode(teapotMesh,litShader,blankTex,blankTex,blankNormal, "Utah Teapot")
    teapot.transform.scale = Vector3(0.25)
    teapot.transform.position = Vector3(-4,0,-1)
    teapot.setParent(rootNode)

    uvTest = RenderNode(cubeMesh,litShader,UVTex,blankTex,blankNormal, "uvTest")
    uvTest.transform.position = Vector3(5,0,0)
    uvTest.setParent(rootNode)
    uvTestTri = RenderNode(testMesh,litShader,UVTex,blankTex,blankNormal, "uvTest")
    uvTestTri.transform.scale = Vector3(0.25)
    uvTestTri.transform.position = Vector3(0,1,0)
    uvTestTri.setParent(uvTest)

    normalTest = RenderNode(cubeMesh,litShader,rockDiffuse,rockSpecular,rockNormal, "normalTest")
    normalTest.transform.position = Vector3(0,0,-5)
    normalTest.transform.scale = Vector3(1.5)
    normalTest.setParent(rootNode)
    ...
    sGround = RenderNode(cubeMesh,litShader,blankTex,blankTex,blankNormal, "Ground")
    sGround.transform.scale = Vector3(10,0.5,10)
    sGround.transform.position = Vector3(0,-0.75,0)
    sGround.setParent(shadowTestZone)
    sBox0 = RenderNode(cubeMesh,litShader,blankTex,blankTex,blankNormal, "Box 0")
    sBox0.transform.position = Vector3(0,0,0)
    sBox0.setParent(shadowTestZone)
    sBox1 = RenderNode(cubeMesh,litShader,blankTex,blankTex,blankNormal, "Box 1")
    sBox1.transform.position = Vector3(1,2,3)
    sBox1.setParent(shadowTestZone)
    sBox2 = RenderNode(cubeMesh,litShader,blankTex,blankTex,blankNormal, "Box 2")
    sBox2.transform.position = Vector3(-3,0.5,2)
    sBox2.setParent(shadowTestZone)
    sBox3 = RenderNode(cubeMesh,litShader,blankTex,blankTex,blankNormal, "Box 3")
    sBox3.transform.position = Vector3(2,0,-1)
    sBox3.setParent(shadowTestZone)
    sBox4 = RenderNode(cubeMesh,litShader,blankTex,blankTex,blankNormal, "Box 4")
    sBox4.transform.position = Vector3(3,1,0)
    sBox4.setParent(shadowTestZone)
    sBox5 = RenderNode(cubeMesh,litShader,blankTex,blankTex,blankNormal, "Box 5")
